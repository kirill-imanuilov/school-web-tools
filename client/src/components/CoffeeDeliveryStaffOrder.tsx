      ...
      <div className='coffee-delivery-staff-order-content'>
        <ul>
          ...
          <li>
            ...
            <strong>{props.orderData.syrop !== 'Без сиропа' && ' ' + props.orderData.syrop.toLowerCase()}</strong>
          </li>
          <li>
            Дополнения:
            <strong>{props.orderData.addition === 'Без дополнений' && ' нет'}</strong>
            <strong>{props.orderData.addition !== 'Без дополнений' && ' ' + props.orderData.addition.toLowerCase()}</strong>
          </li>
          <li>Время заказа: <strong>{props.orderData.orderTime}</strong></li>
          <li>Время доставки: <strong>{props.orderData.deliveryTime}</strong></li>
          <li>Место доставки: <strong>{props.orderData.building.toLowerCase()}, {props.orderData.office} кабинет</strong></li>
        </ul>
        <button onClick={event => handleCourierSentButtonClick(event, props.orderData.id)}>
          Курьер отправлен
        </button>
      </div>
    </div>
